        ...
        <header>
            <nav className={`flex items-center  justify-between fixed z-50 w-screen py-4 px-2 sm:px-8 ${navBgClass}`}>
                <div className={`flex items-center text-xl font-bold ${navTextClass}`}>
                    ...
                    <div className="hidden sm:block">B&rsquo;day Countdown</div>
  <div className="block sm:hidden">B&rsquo;day</div>
  <div className="block sm:hidden">Countdown</div>
                    </div>
                </div>
                <ul className={`flex flex-col space-y-2 items-center ${navBgClass} w-full rounded-md absolute top-16 transition-all duration-500 ease-in-out ${isNavOpen ? 'opacity-100' : 'opacity-0 invisible'}`}>
